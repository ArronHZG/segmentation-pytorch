...
from torch_model.net.nn.DANet.danet import DANet
...
class FCN(nn.Module):
    def __init__(self, num_classes, in_channels, backbone, pretrained, aux_classifier=None):
        ...
        self.pool1_FCNHead = DANet(256, num_classes)
        self.pool2_FCNHead = DANet(512, num_classes)
        self.pool3_FCNHead = DANet(1024, num_classes)
        self.pool4_FCNHead = DANet(2048, num_classes)
        ...
        self.up1 = FCNUpsampling(num_classes, 4, stride=2, padding=1)
        self.up2 = FCNUpsampling(num_classes, 4, stride=2, padding=1)
        self.up3 = FCNUpsampling(num_classes, 4, stride=2, padding=1)
        self.up4 = FCNUpsampling(num_classes, 8, stride=4, padding=2)
    ...
    def forward(self, x):
        ...
        x = self.up1(pool4_same_channel) + pool3_same_channel

        # merge x and pool2  scaling = 1/8
        x = self.up2(x) + pool2_same_channel

        # merge x and pool2  scaling = 1/4
        x = self.up3(x) + pool1_same_channel
    ...
    def get_other_params(self):
        ...
        for i in range(1, 5, 1):
            other_parameters_list.append(getattr(self, f"pool{i}_FCNHead").parameters())
        for i in range(1, 5, 1):
            other_parameters_list.append(getattr(self, f"up{i}").parameters())

        return filter(lambda p: p.requires_grad,
                      chain(*other_parameters_list))
